...
final class NewGiftViewController: BaseViewController {
  ...
  private let giverButton = PlainFavorButton(with: .main("친구 선택", isRight: true))
  private let receiverButton = PlainFavorButton(with: .main("친구선택", isRight: true))
  private lazy var friendLine = self.makeLine()
  private lazy var dateLabel = self.makeTitleLabel("날짜")
  private let dateButton = PlainFavorButton(with: .main("날짜 선택", isRight: false))
